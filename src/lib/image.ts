...
export async function fileToAvatar(file: File, size = AVATAR_SIZE): Promise<string> {
  if (!file.type.startsWith('image/')) throw new Error(messages().accounts.avatar.notImage);
  let bitmap: ImageBitmap;
  try {
    bitmap = await createImageBitmap(file, { imageOrientation: 'from-image' });
  } catch {
    throw new Error(messages().accounts.avatar.unreadable);
  }
  try {
    ...
    if (!ctx) throw new Error(messages().accounts.avatar.cannotResize);
