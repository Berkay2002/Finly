import { plural } from '../plural';

export default {
  page: {
    title: 'Accounts & Financial Position',
    subtitle: 'All your accounts. One clear overview.',
    totalAssets: 'Total in accounts',
    allAccountsCombined: 'All accounts combined',
    cashInBank: 'Cash in bank',
    everydayPlusSavings: 'Everyday + savings accounts',
    emergencySavings: 'Emergency savings',
    monthsOfEssentials: (months: string) => `${months} of essentials`,
    reservedForUnexpected: 'Reserved for the unexpected',
    investments: 'Investments',
    ofAssets: (percent: string) => `${percent} of assets`,
    yourAccounts: 'Your accounts',
    yourAccountsSubtitle: 'Balances are what you tell us. Update them whenever you like.',
    position: 'Your financial position',
    positionSubtitle: 'Money you can spend now, kept apart from money you own but do not intend to spend.',
    everydayMoney: 'Everyday money',
    everydayMoneyHint: 'Spending, salary and joint accounts',
    cashSavings: 'Cash savings',
    cashSavingsHint: 'Savings accounts and cash',
    dedicatedEmergency: 'Dedicated emergency savings',
    dedicatedEmergencyHint: 'Not for everyday spending',
    investmentValue: 'Investment value',
    investmentValueHint: 'Tracked, not spendable',
    otherBalances: 'Other tracked balances',
    home: 'Home',
    homeHint: 'Value entered on the mortgage',
    otherProperty: 'Car & other property',
    otherPropertyHint: 'Value entered on the loan',
    csn: 'CSN',
    csnHint: 'Low interest, paid off over many years',
    loans: 'Loans',
    otherLoans: 'Other loans',
    loansHint: 'Mortgage and other credit',
    netWorth: 'Net worth',
    netWorthHint: 'What you own minus loans',
    excludingCsn: 'Excluding CSN',
    excludingCsnHint: 'Can be lowered if income drops, written off at death',
    ifSoldAfterTax: 'If sold today, after tax',
    ifSoldAfterTaxHint: 'Minus tax on AF gains and this year’s savings tax',
    forecast: 'Forecast with expected returns',
    forecastHint: 'If ISK, KF and cash grow as expected. An expectation, not a promise',
    allocation: 'Account allocation',
    addAccountsToSee: 'Add accounts to see how your money is spread.',
    currencyTotal: (currency: string) => `${currency} total`,
    investmentsShare: (percent: string) => `Investments make up ${percent} of your total assets.`,
    investmentsShareBody: 'Remember they can move in value, so they are kept separate from your runway.',
    noEmergencyTitle: 'No dedicated emergency savings yet',
    noEmergencyBody: 'Marking an account as an emergency fund lets Finly show how many months of essentials it covers.',
    goodShapeTitle: 'Your accounts are in good shape.',
    goodShapeBody: (percent: string) => `${percent} of your assets are cash you can reach quickly.`,
    savingsAccountsShown:
      'Savings and investment accounts show on the Savings page on their own, with the balance and monthly deposit you enter here.',
  },
  editor: {
    noAccounts: 'No accounts yet.',
    accountCount: (n: number) => `${n} ${plural(n, 'account', 'accounts')}`,
    addAccount: 'Add account',
    pickWrapper: '· Pick ISK, KF or AF',
    editDetails: 'Edit details',
    remove: 'Remove',
    addFirst: 'Add your first account',
    editAccount: 'Edit account',
    cancel: 'Cancel',
    save: 'Save',
    interest: (percent: string) => `${percent} interest`,
    expected: (percent: string) => `${percent} expected`,
    type: 'Type',
    name: 'Name',
    institution: 'Bank or provider',
    optional: '(optional)',
    institutionPlaceholder: 'e.g. Swedbank, Avanza',
    bankLogoHint: '(shown next to the account)',
    currentBalance: 'Current balance',
    expectedReturn: 'Expected return',
    interestRate: 'Interest rate',
    yearly: '(yearly)',
    monthlyDeposit: 'Monthly deposit',
    onSavings: 'Shown on the Savings page. Add a target there.',
    goalOnSavings: (goal: string) => `Saved for the goal “${goal}” on the Savings page.`,
    costBasis: 'What you paid',
    costBasisHint: '(anskaffningsvärde, optional)',
    fundShare: 'Held in funds',
    dividendYield: 'Dividend yield',
    shares: '(shares)',
    taxYear: (year: number) => `Tax ${year}`,
    estimate: ' (estimate)',
    underTaxFree: (level: string) => `Under the ${level} tax-free level shared by your ISK and KF.`,
    insurerTakes: (withheld: string, back: string) =>
      `The insurer takes ${withheld}; ${back} comes back in your tax return.`,
    finalTaxNextSpring: 'Comes with your final tax next spring.',
    taxIfSold: 'Tax if sold today',
    lossIfSold: 'Loss if sold today',
    returnAfterTax: 'Return after tax',
    returnAfterTaxBeforeSelling: 'Return after tax, before selling',
    holdings: 'Holdings',
    holdingsHint: 'Prices come from Avanza and Nordnet each time you open Finly.',
    holdingsLater: 'Add what the account holds from its page on Accounts once it is saved.',
    noHoldings: 'No holdings yet. Add what the account holds and its value follows the market.',
    openHoldings: 'Holdings and value',
    holdingCount: (n: number) => `${n} ${plural(n, 'holding', 'holdings')}`,
    addHolding: 'Add holding',
    searchPlaceholder: 'Stock, ETF or fund: name, ticker or ISIN',
    noMatches: 'Nothing found.',
    searchUnavailable: 'Search is unavailable right now. Try again later.',
    cash: 'Cash',
    cashHint: '(not invested)',
    totalValue: 'Total value',
    totalValueHint: '(cash + holdings)',
    quantity: 'Quantity',
    avgPrice: 'Average price',
    removeHolding: 'Remove holding',
    editHolding: 'Correct',
    units: (n: string) => `${n} pcs`,
    avgShort: 'avg',
    today: (percent: string) => `${percent} today`,
    fetchingPrice: 'Fetching price…',
    buyMore: 'Buy more',
    sell: 'Sell',
    price: 'Price',
    all: 'All',
    payFromCash: 'Pay from the account’s cash',
    addToCash: 'Add the money to the account’s cash',
    tradeTotal: 'Total',
    newAvgPrice: 'New average price',
    gainOnSale: 'Gain on this sale',
    lossOnSale: 'Loss on this sale',
    afTaxOnSale: (amount: string) => `About ${amount} in tax (30 %) with next year’s tax return`,
    derivedFromHoldings: 'What you paid and the share in funds follow from your holdings.',
    holdingTypes: { stock: 'Stock', etf: 'ETF', fund: 'Fund', certificate: 'Certificate' },
  },
  detail: {
    back: 'Accounts',
    value: 'Value',
    gain: 'Gain',
    gainSub: (percent: string) => `${percent} on what you paid`,
    noGain: 'Add average prices to see it',
    inHoldings: 'In holdings',
    cash: 'Cash',
    notInvested: 'Not invested',
    ofValue: (percent: string) => `${percent} of the value`,
    pricesFrom: (date: string) => `Prices from ${date}`,
    updatedAt: (time: string) => `Updated ${time}`,
    notUpdated: (date?: string) => `Prices could not be updated.${date ? ` Showing prices from ${date}.` : ''}`,
    refresh: 'Refresh prices',
    onHoldings: (amount: string) => `${amount} on holdings`,
  },
  savingsTax: {
    title: (year: number) => `Tax on savings ${year}`,
    estimate: 'Estimate',
    taxFreeUsed: (used: string, level: string) => `${used} of the ${level} tax-free level used`,
    pickWrappers: 'Pick ISK, KF or AF for your investment accounts to include them',
    interestDividendsFunds: 'Interest, dividends and fund tax',
    details: 'Details',
    /** `prevYear` is left out while the rate is still preliminary. */
    sheetSubtitle: (schablon: string, slr: string, prevYear?: number) =>
      `Standard rate (schablonränta) ${schablon}: government borrowing rate (statslåneränta) ${slr}${prevYear === undefined ? ' today' : ` on 30 Nov ${prevYear}`} + 1`,
    taxFreeLevel: 'Tax-free level, shared by ISK and KF',
    underlagExplained:
      'Measured on the tax base (kapitalunderlag): for an ISK the average of the values on 1 Jan, 1 Apr, 1 Jul and 1 Oct plus a quarter of the year’s deposits; for a KF the value on 1 Jan plus deposits (from July at half).',
    takenDuringYear: 'Taken during the year',
    byBankOrInsurer: 'by the bank or insurer',
    addedToFinalTax: 'Added to your final tax',
    backInFinalTax: 'Back in your final tax',
    finalTaxDue: (year: number) => `spring ${year}, usually due 12 May`,
    totalTax: (year: number) => `Total tax ${year}`,
    afTaxIfSold: 'AF: tax if sold today',
    afTaxIfSoldHint: '30 % of the gain',
    finalTaxInterest:
      'Final tax above 30 000 kr gathers interest from 13 February. Paying the part above 30 000 kr to your tax account by 12 February avoids it.',
    /** `slr` is set when next year's rate is not locked yet. */
    nextYear: (year: number, slr?: string) => `${year}${slr ? ` at today’s government borrowing rate (${slr})` : ''}`,
    about: (amount: string) => `about ${amount}`,
    nextYearPreliminary: 'The rate is locked on 30 November. Balances grow by each account’s expected return and deposits.',
    nextYearLocked: (schablon: string) =>
      `Standard rate ${schablon}. Balances grow by each account’s expected return and deposits.`,
    footnote: (date?: string) =>
      `An estimate from the balances you enter, for one person. Government borrowing rate from Riksgälden${date ? `, ${date}` : ''}. Realized gains and losses on AF are not tracked.`,
    iskRow: (underlag: string) => `Tax base ${underlag} · with your final tax next spring`,
    kfRowRefund: (underlag: string, withheld: string, back: string) =>
      `Tax base ${underlag} · insurer takes ${withheld}, ${back} comes back`,
    kfRow: (underlag: string) => `Tax base ${underlag} · taken by the insurer`,
    afFunds: (amount: string) => `Funds ${amount} on 1 Jan`,
    afIfSold: (amount: string) => `${amount} if sold today`,
    afLoss: (amount: string) => `loss worth up to ${amount} if sold`,
    unknownRow: 'Pick ISK, KF or AF to include it',
    cashRow: 'Interest tax, taken by the bank',
  },
  avatar: {
    change: 'Change photo',
    add: 'Add photo',
    remove: 'Remove photo',
    unusable: 'That image could not be used.',
    notImage: 'Choose an image file (JPEG, PNG, WebP or HEIC).',
    unreadable: 'That image could not be read. Try a JPEG or PNG.',
    cannotResize: 'This browser cannot resize images.',
  },
};
