...
export function HouseholdFoodEstimator({
  ...
  return (
    <div>
      <div className="mb-1 text-[12.5px] font-medium text-ink-soft">Who do you buy food for?</div>
        ...
        {shown.map((m, i) => (
          <li key={m.id} className="flex flex-wrap items-center gap-2">
            ...
            <TogglePill
              ...
              options={[
                { value: 'home', label: 'Lunch at home' },
                { value: 'away', label: 'Lunch out' },
              ]}
            />
            <IconButton
              icon={X}
      ...
      <p className="mt-2 text-[11.5px] text-muted">
        "Lunch out" means weekday lunch at school, or bought at work. A packed lunch from home counts as home.
      </p>

      <div className="mt-3 rounded-lg bg-card px-3 py-2.5">
        <div className="flex items-baseline justify-between gap-3">
          <span className="text-[12.5px] text-ink-soft">Konsumentverket {cost.year}</span>
          <span className="tabular text-[15px] font-semibold text-ink">
            {money(cost.monthly)}
            <span className="ml-1 text-[12px] font-normal text-muted">a month</span>
          </span>
        </div>
        <div className="tabular text-right text-[12px] text-muted">≈ {money(monthlyToWeekly(cost.monthly))} a week</div>
        <p className="mt-1.5 text-[11.5px] text-muted">
          A reasonable level when cooking at home from their menu, not what households actually spend. Treat it as a
          starting point and adjust once you see your real months.
          {cost.adultsLunchingOut > 0 && ' Lunches bought at work are not included: add them as Work lunches.'}
        </p>
      </div>
