...
- **ExpenseItem** — category (home | living | transport | finance | leisure | planned), subcategory, amount, frequency, optional `nextDate` for non-monthly items, and three independent classification flags from PRD §14: `fixed`, `essential`, `committed`. Tags (`car`, `subscription`, `insurance`, `utility`) power the deeper analyses (`debt` only survives on old plans; loans are their own model). `includedElsewhere` marks utilities bundled into rent so they are visible but excluded from totals.
  ...
  - `occurrences` (`times` per `week` | `month`) prices an item per purchase: `amount` and `range` are one lunch, one coffee, one takeaway, and `frequency` mirrors `per`. Monthly = price × times (× 52/12 for weekly). Everyday taxonomy items (groceries, cafés, fuel, public transport…) offer only Each time, Weekly and Monthly, start in a sensible cadence (groceries weekly, work lunches 5 a week) and are never asked for as bills.
...
- **foodSpend** — what was really spent on Food & drink per month (`YYYY-MM` → `{ amount, asOf? }`), read off the bank app. With `asOf` it is a running total for the pace; without (or dated the last day) it covers the whole month. Everyday spending has no invoice, so the month gets one total instead of a bill per item.
...
| Actuals (`metrics.actuals`) | for the viewed month: `pending` variable monthly items without a bill (everyday spending excluded), `confirmed` items with one, `variance` = Σ(actual − typical). A food total for the whole month replaces every food item's estimate and adds (spent − planned food). Safe to spend subtracts the variance; the baseline plan is untouched |
| Food (`metrics.food`) | Food & drink items per month / week (× 12/52) / day (× 12/365), at home vs eating out, flexible part, `nudges` (one fewer purchase of an optional per-purchase item, biggest saving first), `month` pace (expected by now = planned × day ÷ days; projected = spent ÷ share of month; left per remaining day) and `history` (average of the last three complete months; a gap is flagged from two months when it exceeds 5 % and 100 kr) |
...
| `/income`, `/home`, `/living`, `/transport`, `/finance`, `/leisure`, `/planned` | Editable section pages with a stat strip and the same line-item editor as onboarding. Living Costs adds a Food & drink card (month/week/day, at home vs eating out, small changes, groceries estimate from the household) and a food-this-month card (running total and pace, last month's total, your months vs the plan) |
...
| `/planning` | Can I afford this?, income-change scenario, daily/weekly allowance with the food part split out |
